...
struct HostEnvironment {
  ...
  private static func availableCapacity(at url: URL) -> UInt64 {
    ...
    let capacity = try? probeURL.resourceValues(forKeys: [
...
final class AdmissionPolicy {
  ...
  private func isoIssues(for isoURL: URL, host: HostEnvironment) -> [AdmissionIssue] {
    ...
    if compatibility(for: isoURL) == .unknown,
      !issues.contains(where: { $0.code == .isoArchitectureMismatch })
    {
  ...
  private func resourceIssues(for resources: VMResourceConfiguration, host: HostEnvironment)
    -> [AdmissionIssue]
  {
    var issues: [AdmissionIssue] = []
