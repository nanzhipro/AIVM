...
struct VMBundleLayout: Equatable {
  ...
  func bundleURL(for id: UUID) -> URL {
    rootDirectory.appendingPathComponent(
      "\(id.uuidString).\(Self.bundleExtension)", isDirectory: true)
  }
...
final class VMBundleStore {
  ...
  init(
    rootDirectory: URL = VMBundleStore.defaultRootDirectory(), fileManager: FileManager = .default
  ) {
  ...
  func createBundle(for metadata: VMMetadata) throws -> URL {
    ...
    try fileManager.createDirectory(
      at: layout.logsURL(for: metadata.id), withIntermediateDirectories: true)
    try save(metadata)
    return bundleURL
  }
  ...
  private func validateBundleURL(_ bundleURL: URL) throws {
    guard layout.contains(bundleURL), bundleURL.pathExtension == VMBundleLayout.bundleExtension
    else {
      throw VMBundleStoreError.unsafeBundlePath(bundleURL)
    }
  }
