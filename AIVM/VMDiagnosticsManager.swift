...
final class VMDiagnosticsManager: VMDiagnosticsProviding {
    ...
    private func makeSnapshot(for metadata: VMMetadata, host: HostEnvironment) -> VMDiagnosticSnapshot {
        VMDiagnosticSnapshot(
            ...
            installMediaName: metadata.installMediaPath.map { URL(fileURLWithPath: $0).lastPathComponent },
            artifacts: VMDiagnosticSnapshot.Artifacts(
                bundleName: metadata.bundleName,
                diskImageExists: fileManager.fileExists(atPath: store.layout.diskImageURL(for: metadata.id).path),
                machineIdentifierExists: fileManager.fileExists(atPath: store.layout.machineIdentifierURL(for: metadata.id).path),
