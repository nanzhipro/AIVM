...
final class VMLifecycleController: VMLifecycleControlling {
  typealias VirtualMachineFactory =
    @MainActor (VZVirtualMachineConfiguration) -> VirtualMachineOperating
  ...
  func start(metadata: VMMetadata) async throws -> VMMetadata {
    ...
    logger.info(
      "VM start requested id=\(metadata.id.uuidString, privacy: .public) state=\(metadata.state.rawValue, privacy: .public)"
    )
    ...
    do {
      ...
      logger.info(
        "VM start succeeded id=\(metadata.id.uuidString, privacy: .public) state=\(updated.state.rawValue, privacy: .public)"
      )
      return updated
    } catch {
  ...
  func stop(metadata: VMMetadata) async throws -> VMMetadata {
    ...
    logger.info(
      "VM stop requested id=\(metadata.id.uuidString, privacy: .public) state=\(metadata.state.rawValue, privacy: .public)"
    )
