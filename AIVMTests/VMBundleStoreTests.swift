...
final class VMBundleStoreTests: XCTestCase {
  ...
  func testBundleLayoutUsesStableNames() {
    ...
    let layout = VMBundleLayout(
      rootDirectory: URL(fileURLWithPath: "/tmp/AIVMTests", isDirectory: true))

    XCTAssertEqual(
      layout.bundleURL(for: id).lastPathComponent, "AAAAAAAA-BBBB-CCCC-DDDD-EEEEEEEEEEEE.vmbundle")
  ...
  func testCreateLoadListUpdateAndDeleteBundle() throws {
    ...
    XCTAssertTrue(
      FileManager.default.fileExists(atPath: store.layout.configURL(for: metadata.id).path))
    XCTAssertTrue(
      FileManager.default.fileExists(atPath: store.layout.logsURL(for: metadata.id).path))
    XCTAssertFalse(
      FileManager.default.fileExists(atPath: store.layout.diskImageURL(for: metadata.id).path))
